(function() {
  ...
  // ── Sliders shelving (refs guardadas para usarlas en startSound) ──
  ...
  function updateShelvingUI() {
    ...
    shelvingGainVal.textContent = gainDb;
    shelvingFreqVal.textContent = freq;
    // Indicador visual: cuanto más atenuación, más llena la barra
    const pct = ((gainDb * -1) / 12) * 100;
    shelvingFill.style.width = pct + '%';
    // Si el filtro ya existe y está activo, actualizar en tiempo real
    if (shelvingFilter) {
      shelvingFilter.gain.setTargetAtTime(gainDb, audioCtx.currentTime, 0.05);
      shelvingFilter.frequency.setTargetAtTime(freq, audioCtx.currentTime, 0.05);
    }
  }

  shelvingGainSlider.addEventListener('input', updateShelvingUI);
  shelvingFreqSlider.addEventListener('input', updateShelvingUI);
  ...
  function analyzeAndDisplay() {
    ...
    drawRadar(m);
    if (isPlaying && oscillator) oscillator.frequency.setTargetAtTime(m.freq, audioCtx.currentTime, 0.1);
  }
  ...
  // ////////////////////////////////////////////////////////////
  // AUDIO — cadena: oscillator → gainNode → shelvingFilter → destination
  // ////////////////////////////////////////////////////////////

  function startSound() {
    if (!hasImage || !currentMetrics) return;
    ...
    oscillator.frequency.value = currentMetrics.freq;

    // Ganancia principal
    gainNode = audioCtx.createGain();
    gainNode.gain.setValueAtTime(0, audioCtx.currentTime);
    gainNode.gain.linearRampToValueAtTime(0.18, audioCtx.currentTime + 0.4);

    // Filtro high-shelf para atenuar agudos
    shelvingFilter = audioCtx.createBiquadFilter();
    shelvingFilter.type = 'highshelf';
    shelvingFilter.frequency.value = parseFloat(shelvingFreqSlider.value);
    shelvingFilter.gain.value      = parseFloat(shelvingGainSlider.value);

    // Cadena: osc → gain → shelf → out
    oscillator.connect(gainNode);
    gainNode.connect(shelvingFilter);
    shelvingFilter.connect(audioCtx.destination);

    oscillator.start();
    isPlaying = true;
    document.getElementById('btnPlay').classList.add('playing');
    announce('Reproduciendo sonificación. Frecuencia ' + Math.round(currentMetrics.freq) + ' hercios.');
  }
  ...
  function stopSound() {
    ...
    setTimeout(() => {
      ...
      oscillator = null; gainNode = null; shelvingFilter = null; audioCtx = null;
    }, 500);
    isPlaying = false;
